...
def start_timer():
    ...
    if params["reps_left"] == 0 and params["breaks_left"] == 0:
        # when whole session is complete
        reset()
...
# checkmarks
check_label = tk.Label(text="", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 16), width=10)
check_label.grid(row=3, column=1)

# right column
reset_btn = tk.Button(text="Reset", fg="grey", bg=PINK, font=(FONT_NAME, 12), width=6, relief="ridge", command=reset)
reset_btn.grid(row=2, column=2, padx=0)
